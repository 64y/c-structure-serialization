...
target_library_file="$PWD/libcstructureserialization.${target_bin}.${target_bin_hash}.${target_bin_birth_time}"
...
cp /home/pavel/Workspace/c-structure-serialization/res/* ${target_bin_library_dir} -r
...
valgrind -s --leak-check=full --track-origins=yes /home/pavel/Workspace/c-structure-serialization/bin/generate_codecs ${target_bin_library_dir_src} ${structures_dir}

gcc -shared -nostartfiles -fPIC -I ${target_bin_library_dir_src} -o ${target_library_file} $(find ${target_bin_library_dir_src} -type f -name "*.c")
